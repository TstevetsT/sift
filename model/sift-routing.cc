                   ...
                   Ptr<Ipv4Route> route)
{
  ...
  if (protocol == 1)
  ...
  else
    {
      ...
      /* Timer live=5; // The time a packet stays in the buffer
         m_sentPacket.insert(p,live);  // Keep a copy of sent packet */
                            ...
                            uint8_t protocol)
{
  ...
  NS_ASSERT_MSG (!m_downTarget.IsNull (), "Error, SiftRouting cannot send downward");
  NS_LOG_DEBUG (std::cout << " Packet content in ForwardPacket module node (" << this->GetNode ()->GetId () << ") with IP address: "
...
SiftRouting::ScheduleTimer (Ptr<Packet> packet, Time delay, Ipv4Address source, uint8_t protocol)
{
  ...
  NS_LOG_DEBUG ("\nIn ScheduleTimer of node (" << this->GetNode ()->GetId () << ") with IP address: "
                                               << this->GetIPfromID (this->GetNode ()->GetId ())
                                               << ", Delay:" << delay << ", Protocol: " << (uint32_t)protocol);
  ...
  for (std::map<Ptr<Packet>,Timer >::iterator it = m_timer.begin (); it != m_timer.end (); ++it)
    {
      ...
      //std::cout<< *(it->first)<<" State: " << (it->second).GetState() <<" Current Time: "<< Simulator::Now()<< "\n";
      if (timerSift.GetSourceAddress () == tempSift.GetSourceAddress ()
          && timerSift.GetDestAddress () == tempSift.GetDestAddress ()
          && timerSift.GetHeaderSeqNo () == tempSift.GetHeaderSeqNo ())
        {
                      ...
                      Ptr<Ipv4Interface> incomingInterface)
{
      ...
      else
        {
          ...
          if (tempSift.GetSourceAddress () == sourceAddress
              ...
              && tempSift.GetHeaderSeqNo () == headerSeqNo)
            {
              ...
              //*** std::cout<< " Duplicate packet received in node "<< m_node->GetId() << std::endl;
  ...
  if (destAddress.IsEqual (zero))
    {
      std::cout << "--------------- Receive Module- 0.0.0.0 is detected. The header is checking -----------\n";
      std::cout << "Source Address= " << sourceAddress << std::endl;
      std::cout << "Destination Address= " << destAddress << std::endl;
      std::cout << "SourceXLoc= " << sourceXLoc << std::endl;
      std::cout << "SourceYLoc= " << sourceYLoc << std::endl;
      std::cout << "last Source Xlox= " << lastsourceXLoc << std::endl;
      std::cout << "last source Yloc= " << lastsourceYLoc << std::endl;
      std::cout << "dest Xloc= " << destXLoc << std::endl;
      std::cout << "dest YLoc= " << destYLoc << std::endl;
  ...
  if (m_mainAddress == destAddress)   // This is the final destination of the packet
    {
      ...
      //Ptr<Ipv4L3Protocol> l3p;
      //l3p->LocalDeliver(packet,ip,incomingInterface);
      if (nextProto != 0)
        {
  ...
  else   // This is not final destination, forward the packet to next hop
    {
      ...
      // TODO This is the way to get all the nodes positions
      ...
      if (Y == 0)
        {
          ...
          try
            ...
            {
              std::cout << "**** Exception in SIFT routing (Y==0) calculation in node ("
                        << this->GetNode ()->GetId () << ") with IP address: "
                        << this->GetIPfromID (this->GetNode ()->GetId ())
                        << "\n\t Current coordinate (x,y)= (" << currentX << ", " << currentY
                        << ")\n\t Source(x,y)= (" << sourceXLoc << ", " << sourceYLoc
                        << ")  Destination(x,y)= (" << destXLoc << ", " << destYLoc
      ...
      else
        {
          try
            ...
            {
              std::cout << " **** Exception in SIFT routing (slope=Y/X) calculation in node ("
                        << this->GetNode ()->GetId () << ") with IP address: "
                        << this->GetIPfromID (this->GetNode ()->GetId ())
                        << "\n\t Current coordinate (x,y)= (" << currentX << ", " << currentY
                        << ")\n\t Source(x,y)= (" << sourceXLoc << ", " << sourceYLoc
...
void SiftRouting::PrintSiftHeader (SiftHeader siftHeader, GeographicTuple geoTuple)
{
  ...
  std::cout << "Current Node IP address= " << currentAddress << std::endl;
  std::cout << "Source Address= " << sourceAddress << std::endl;
  std::cout << "Destination Address= " << destAddress << std::endl;
  std::cout << "SourceXLoc= " << sourceXLoc << std::endl;
  std::cout << "SourceYLoc= " << sourceYLoc << std::endl;
  std::cout << "CurrentXLoc= " << (int)geoTuple.xcoord << std::endl;
  std::cout << "CurrentYLoc= " << (int)geoTuple.ycoord << std::endl;
  std::cout << "last Source Xlox= " << lastsourceXLoc << std::endl;
  std::cout << "last source Yloc= " << lastsourceYLoc << std::endl;
  std::cout << "dest Xloc= " << destXLoc << std::endl;
  std::cout << "dest YLoc= " << destYLoc << std::endl;
